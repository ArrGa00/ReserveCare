...
입원 관리 시스템은 구급 대원과 병원의 환자 이송 연결 및 운영을 효율적으로 관리하기 위해 설계하였다. </br>
...
7. 환자 퇴원시 가용 병상 정보를 업데이트한다.

###  ▶ 비기능적 요구사항
1. 트랜젝션
    - 가용 병상이 존재하지 않는다면, 병원의 승인은 반영되지 않는다.
    - 구급대원의 병원 예약은 병원이 승인 이벤트를 발생하기 전까지 가능하다.
2. 장애격리
   - 병원 예약 서비스가 동작하지 않더라도 병원 데이터 서비스와 환자 서비스는 24시간 제공할 수 있다.
3. 성능
   - 사용자는 병원 목록을 확인하고 병원 가용 병상 정보를 확인할 수 있어야 한다. CQRS
...
- 이벤트스토밍 과정 중 도출된 잘못된 도메인 이벤트들을 걸러내는 작업을 수행한다.
- 하단 세 가지의 이벤트는 event가 아닌 policy의 개념이거나, 불필요한 행동이므로 이벤트에서 삭제한다.
